...
void embed_into_carrier(char* message, int msg_size, FILE* carrier, FILE* output) {
    ...
    // embed size
    uint8_t big_endian_size[4] = {msg_size >> 24, msg_size >> 16, msg_size >> 8, msg_size};
    uint32_t prueba = big_endian_size[0] << 24 | big_endian_size[1] << 16 | big_endian_size[2] << 8 | big_endian_size[3];
